...
var splitIncCount = parseInt(env("CISCO_SPLIT_INC"), 10);
if (isNaN(splitIncCount) || splitIncCount < 0) {
    splitIncCount = 0;
}

function appendSplitInclude(addr, mask, maskLen) {
    env("CISCO_SPLIT_INC_" + splitIncCount + "_ADDR") = addr;
    env("CISCO_SPLIT_INC_" + splitIncCount + "_MASK") = mask;
    env("CISCO_SPLIT_INC_" + splitIncCount + "_MASKLEN") = maskLen;
    splitIncCount += 1;
}
...
case "connect":
    ...
    if (env("CISCO_SPLIT_INC")) {
        for (var j = 0 ; j < parseInt(env("CISCO_SPLIT_INC")); j++) {
            var network = env("CISCO_SPLIT_INC_" + j + "_ADDR");
            var netmask = env("CISCO_SPLIT_INC_" + j + "_MASK");
            var netmasklen = env("CISCO_SPLIT_INC_" + j + "_MASKLEN");
    ...
    if (env("CISCO_SPLIT_EXC")) {
        for (var k = 0 ; k < parseInt(env("CISCO_SPLIT_EXC")); k++) {
            var excnetwork = env("CISCO_SPLIT_EXC_" + k + "_ADDR");
            var excnetmask = env("CISCO_SPLIT_EXC_" + k + "_MASK");
            var excnetmasklen = env("CISCO_SPLIT_EXC_" + k + "_MASKLEN");
            run("route add " + excnetwork + " mask " + excnetmask + " " + gw4);
            echo(INFO, "Configured Legacy IP split-exclude route: " + excnetwork + "/" + excnetmasklen);
        }
    }
    ...
    if (env("INTERNAL_IP6_ADDRESS")) {
        ...
        if (env("CISCO_IPV6_SPLIT_INC")) {
            for (var m = 0 ; m < parseInt(env("CISCO_IPV6_SPLIT_INC")); m++) {
                var network6 = env("CISCO_IPV6_SPLIT_INC_" + m + "_ADDR");
                var netmasklen6 = env("CISCO_IPV6_SPLIT_INC_" + m + "_MASKLEN");
                run("netsh interface ipv6 add route " + network6 + "/" +
                    netmasklen6 + " " + env("TUNIDX") + " store=active");
                echo(INFO, "Configured IPv6 split-include route: " + network6 + "/" + netmasklen6);
            }
        } else {
...
case "disconnect":
    ...
    var vpngw2 = env("VPNGATEWAY");
    if (vpngw2.match(/:/g)) {
        echo(INFO, "Removing explicit route to IPv6 VPN gateway " + vpngw2);
        run("netsh interface ipv6 delete route " + vpngw2 + "/128 " + getDefaultGateway6());
    } else {
        echo(INFO, "Removing explicit route to IPv4 VPN gateway " + vpngw2);
        run("route delete " + vpngw2 + " mask 255.255.255.255");
    }
    ...
    if (env("CISCO_SPLIT_EXC")) {
        ...
        for (var n = 0 ; n < parseInt(env("CISCO_SPLIT_EXC")); n++) {
            var delnetwork = env("CISCO_SPLIT_EXC_" + n + "_ADDR");
            var delnetmask = env("CISCO_SPLIT_EXC_" + n + "_MASK");
            run("route delete " + delnetwork + " mask " + delnetmask);
        }
    }
